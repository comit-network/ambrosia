...
export class LedgerClient {
  ...
  async signBitcoinTransaction(
    psbt: Psbt,
    previousTransactions: { tx: string; index: number }[]
  ): Promise<string> {
  ...
  async getBitcoinWalletDescriptors(
    accountIndex,
    network: Network
  ): Promise<Descriptors> {
    return this.ipc.invoke(
      GET_BITCOIN_WALLET_DESCRIPTORS,
      accountIndex,
      network
    );
  }
...
export class LedgerServer {
  ...
  start() {
    this.ipc.handle(
      SIGN_BITCOIN_TRANSACTION,
      (_, psbt, previousTransactions) => {
        return sign(Psbt.fromBase64(psbt), previousTransactions);
      }
    );
    this.ipc.handle(
      GET_BITCOIN_WALLET_DESCRIPTORS,
      async (_, accountIndex, network) => {
    ...
    this.ipc.handle(
      SIGN_ETHEREUM_TRANSACTION,
      async (_, tx, accountIndex, chainId) => {
        const transport = await TransportNodeHid.open();
        const eth = new AppEth(transport);
...
async function sign(
  ...
  const inputs = previousTransactions.map(({ tx, index }) => {
    return [btc.splitTransaction(tx, true), index];
  });
  const derivationPaths = psbt.data.inputs.map(
    input => input.bip32Derivation[0].path
  );
...
function makeFingerPrint(publicKey) {
  return crypto
    .hash160(Buffer.from(compressPublicKey(publicKey), 'hex'))
    .toString('hex')
    .substr(0, 8);
}
...
async function getWalletDescriptors(
  change: number,
  account: number,
  network: Network
) {
  ...
  const fingerprint = makeFingerPrint(master.publicKey);

  const coinType = network === 'main' ? 0 : 1;
  const networkVersion =
    network === 'main' ? createXPub.mainnet : createXPub.testnet;

  const path = `/84'/${coinType}'/${account}'`;
